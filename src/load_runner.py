    ...
    def start(self):
        ...
        while the_time < stop:
            ...
            if self.ramp_steps > 1:
                logging.debug("threads_started = %s" % threads_started)
                logging.debug("ramp_count = %s" % ramp_count)
                if threads_started < self.max_threads \
                and ramp_count >= self.ramp_interval:
                    logging.debug("LoadRunnerManager: adding %s threads" % initial_threads)
            ...
            the_time = time.time()
            logging.info('thread pool queue size = %s' %
                  int(pool.getApproximateQueueSize()))
            logging.info("LoadRunnerManager: time remaining: %s seconds" %
                  (stop - the_time))
        pool.stop()
    ...
    def run(self):
        ...
        if params != None:
            try:
                ...
                logging.error('LoadRunner.run exception: %s' % str(inst))
                time.sleep(float(self.throttle_rate))
...
class ThreadPool:
    ...
    def addThreads(self, num_threads=1):
        ...
        logging.debug('ThreadPool: adding %s threads' % num_threads)
